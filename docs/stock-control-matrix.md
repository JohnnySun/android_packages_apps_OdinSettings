...
| Mapping service boundary | TouchMapping DEX names `IServerApi`, `MappingProxy`, get/set gamepad config transactions, and input listeners. GameAssistant embeds the client-side SDK model. | Separate UI/client state from a service/adapter boundary with explicit result types and capability status. | High | Framework | The UI calls `com.ayn.controller.IOdinController/default` asynchronously, preserves exact set failures, and synchronizes only read-back profile state. |
...
| Fan control | Live validation mapped `/sys/class/gpio5_pwm2/{state,duty,period,speed}` with a required `50000 ns` period. `state=1` followed by a `25000 ns` PWM high-time produced a tach value of `3300`; cleanup restored `state=0` and the `10000 ns` baseline high-time. | Allow only Off (`state=0`, then `10000`), Quiet (`state=1`, then `5000`), and Sport (`state=1`, then `25000`). The private `com.ayn.fan.IOdinFan/default` service owns device/path validation, writes, readback, and fail-closed cleanup. The app must use full confirmed snapshots, keep Binder work off the UI thread, and never retry an ambiguous `setMode`. Treat tach as pulses times 300, not guaranteed RPM. | Live-validated path and bounded values | Privileged I/O | Implemented as a private AIDL client. The app contains no fan JNI or direct sysfs access and exposes no arbitrary tuning or Custom mode. |
...
4. The system-profile UI remains a client of a separately reviewed service. It
   may request only the two allowlisted profiles, persists only read-back state,
   and treats unknown hardware, service loss, and unconfirmed writes as failures.
