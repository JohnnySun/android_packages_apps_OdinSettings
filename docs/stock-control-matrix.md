...
| Fan control | Native JNI names `setFanEnabled` and `setFanSpeed`; stock settings inventory observes fan-related setting candidates but no validated end-to-end path. | Expose no writable fan control until thermal bounds, actual node/service, read-back, and restore behavior are proven. | High that stock can request it; low on safe path | Privileged I/O | Disabled read-only status only. |
