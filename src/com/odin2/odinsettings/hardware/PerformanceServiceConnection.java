 ...
 */
public final class PerformanceServiceConnection<T> {
    ...
    public interface Call<T> {
        PerformanceControlResult call(T service) throws RemoteFailure;
    }
    ...
    public PerformanceServiceConnection(Connector<T> connector) {
        if (connector == null) {
            throw new IllegalArgumentException("Performance service connector is required");
        }
        this.connector = connector;
    }

    /** Reads may reconnect and retry, because a read has no side effect. */
    public PerformanceControlResult read(Call<T> call) {
        if (call == null) {
            throw new IllegalArgumentException("Performance service read call is required");
        }
        ...
        return PerformanceControlResult.failure(
                PerformanceControlResult.Code.UNAVAILABLE, null);
    }

    /** Writes are attempted once, so a mode change cannot be applied twice. */
    public PerformanceControlResult write(Call<T> call) {
        if (call == null) {
            throw new IllegalArgumentException("Performance service write call is required");
        }
        T service = connectQuietly();
        if (service == null) {
            return PerformanceControlResult.failure(
